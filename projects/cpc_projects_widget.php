...
class CPC_Projects_Recent_Tasks_Widget extends WP_Widget {
    ...
    public function __construct() {
        parent::__construct(
            ...
            __('(Projects) Meine Tasks', 'cp-community'),
            array(
                'classname' => 'cpc_projects_recent_tasks_widget',
                'description' => __('Zeigt die letzten Tasks des aktuell angemeldeten Nutzers.', 'cp-community'),
            )
        );
    }
    ...
    public function widget($args, $instance) {
        $title = !empty($instance['title']) ? (string)$instance['title'] : __('Meine Tasks', 'cp-community');
        ...
        if (!is_user_logged_in()) {
            echo '<div class="cpc_projects_widget_no_tasks">'.esc_html__('Bitte anmelden, um Deine Tasks zu sehen.', 'cp-community').'</div>';
        ...
        if (empty($tasks)) {
            echo '<div class="cpc_projects_widget_no_tasks">'.esc_html__('Keine Tasks gefunden.', 'cp-community').'</div>';
            echo $args['after_widget'];
            return;
        }
        ...
        foreach ($tasks as $task) {
            ...
            if (!empty($task->deadline)) {
                ...
                if ($deadline_ts > $now_ts) {
                    $time_label = sprintf(
                        __('Faellig: %s', 'cp-community'),
                        human_time_diff($now_ts, $deadline_ts)
                    );
                } else {
                    $time_label = __('UEBERFAELLIG', 'cp-community');
                }
            } else {
                $time_label = __('Keine Frist', 'cp-community');
            }
    ...
    public function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php esc_html_e('Titel:', 'cp-community'); ?></label>
            <input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" />
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('task_number')); ?>"><?php esc_html_e('Anzahl Tasks:', 'cp-community'); ?></label>
