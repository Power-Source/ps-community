...
function cpc_projects_render_events_html($project_id, $limit = 60) {
    ...
    if ($project_id <= 0) {
        return '<p>' . esc_html__('Noch keine Events vorhanden.', 'cp-community') . '</p>';
    }
    ...
    foreach ($activities as $activity) {
        ...
        $when = sprintf(__('vor %s', 'cp-community'), human_time_diff($ts, current_time('timestamp', 1)));
        ...
        if (!empty($target_url)) {
            $html .= '<div><a href="' . esc_url($target_url) . '">' . esc_html__('Zum Eintrag', 'cp-community') . '</a></div>';
        }
        $html .= '</li>';
    }
    $html .= '</ul>';
...
function cpc_projects_render_single_project_html($project_id, $args = array()) {
    ...
    if (!empty($args['back_url'])) {
        $html .= '<p class="cpc_projects_back_link"><a href="' . esc_url($args['back_url']) . '">&larr; ' . esc_html__('Alle Projekte', 'cp-community') . '</a></p>';
    }
    ...
    $html .= '<button type="button" class="cpc_projects_single_nav_link is-active" data-target="overview">' . esc_html__('Uebersicht', 'cp-community') . '</button>';
    $html .= '<button type="button" class="cpc_projects_single_nav_link" data-target="tasks">' . esc_html__('Tasks', 'cp-community') . '</button>';
    $html .= '<button type="button" class="cpc_projects_single_nav_link" data-target="activity">' . esc_html__('Aktivitaet', 'cp-community') . '</button>';
    if ($can_manage) {
        $html .= '<button type="button" class="cpc_projects_single_nav_link" data-target="settings">' . esc_html__('Einstellungen', 'cp-community') . '</button>';
    }
    $html .= '</div>';
    ...
    if ((int)$progress['total'] > 0) {
        ...
        $html .= '<div class="cpc_projects_ataglance_item"><span class="cpc_projects_ataglance_num">' . (int)$progress['total'] . '</span><span class="cpc_projects_ataglance_label">' . esc_html__('Gesamt', 'cp-community') . '</span></div>';
        $html .= '<div class="cpc_projects_ataglance_item"><span class="cpc_projects_ataglance_num cpc_projects_ataglance_done">' . (int)$progress['completed'] . '</span><span class="cpc_projects_ataglance_label">' . esc_html__('Erledigt', 'cp-community') . '</span></div>';
        $html .= '<div class="cpc_projects_ataglance_item"><span class="cpc_projects_ataglance_num cpc_projects_ataglance_open">' . (int)$progress['remaining'] . '</span><span class="cpc_projects_ataglance_label">' . esc_html__('Offen', 'cp-community') . '</span></div>';
    ...
    if ($can_manage) {
        ...
        $html .= '<label>' . esc_html__('Titel', 'cp-community') . '</label>';
        $html .= '<input type="text" name="title" value="' . esc_attr(get_the_title($project_id)) . '" required />';
        $html .= '<label>' . esc_html__('Beschreibung', 'cp-community') . '</label>';
        ...
        $html .= '<option value="public"' . selected($current_status, 'public', false) . '>' . esc_html__('Oeffentlich', 'cp-community') . '</option>';
        $html .= '<option value="members"' . selected($current_status, 'members', false) . '>' . esc_html__('Nur Mitglieder', 'cp-community') . '</option>';
        $html .= '<option value="private"' . selected($current_status, 'private', false) . '>' . esc_html__('Privat', 'cp-community') . '</option>';
        $html .= '</select>';
        $html .= '</div>';
        $html .= '<p><button type="submit" class="cpc_button">' . esc_html__('Projekt aktualisieren', 'cp-community') . '</button></p>';
        $html .= '</form>';
        $html .= '<hr class="cpc_projects_settings_divider" />';
        $html .= '<div class="cpc_projects_settings_danger">';
        $html .= '<p class="cpc_projects_settings_danger_hint">' . esc_html__('Projekt und alle Tasks dauerhaft loeschen. Diese Aktion kann nicht rueckgaengig gemacht werden.', 'cp-community') . '</p>';
        $html .= '<button type="button" class="cpc_button cpc_button_danger cpc_projects_delete_project_btn" data-project-id="' . (int)$project_id . '">' . esc_html__('Projekt loeschen', 'cp-community') . '</button>';
        $html .= '</div>';
        $html .= '</div>';
        $html .= '</section>';
    }
...
function cpc_projects_render_create_form($component, $component_id) {
    ...
    $html .= '<summary class="cpc_button cpc_projects_create_summary">'.esc_html__('Projekt hinzufuegen', 'cp-community').'</summary>';
    ...
    $html .= '<option value="public">'.esc_html__('Oeffentlich', 'cp-community').'</option>';
    $html .= '<option value="members">'.esc_html__('Nur Mitglieder', 'cp-community').'</option>';
    $html .= '<option value="private">'.esc_html__('Privat', 'cp-community').'</option>';
    $html .= '</select>';
    $html .= '</div>';
    $html .= '<p><button type="submit" class="cpc_button">'.esc_html__('Projekt erstellen', 'cp-community').'</button></p>';
    $html .= '</form>';
    $html .= '</details>';
...
function cpc_projects_render_task_priority_label($priority) {
    ...
    if ($priority >= 3) {
        return __('Kritisch', 'cp-community');
    }
    if ($priority === 2) {
        return __('Hoch', 'cp-community');
    }
    return __('Normal', 'cp-community');
}
...
function cpc_projects_render_task_panel($project_id) {
    ...
    $html .= '<h5 class="cpc_projects_task_heading">'.esc_html__('Tasks', 'cp-community').'</h5>';

    // Compact filter bar
    $html .= '<div class="cpc_projects_task_filters_bar">';
    $html .= '<input type="text" class="cpc_projects_task_filter_text" placeholder="'.esc_attr__('Suchen...', 'cp-community').'" />';
    $html .= '<select class="cpc_projects_task_filter_status">';
    $html .= '<option value="all">'.esc_html__('Status', 'cp-community').'</option>';
    $html .= '<option value="open" selected="selected">'.esc_html__('Offen', 'cp-community').'</option>';
    $html .= '<option value="done">'.esc_html__('Erledigt', 'cp-community').'</option>';
    $html .= '</select>';
    $html .= '<select class="cpc_projects_task_filter_priority">';
    $html .= '<option value="all">'.esc_html__('Priorität', 'cp-community').'</option>';
    $html .= '<option value="1">'.esc_html__('Normal', 'cp-community').'</option>';
    $html .= '<option value="2">'.esc_html__('Hoch', 'cp-community').'</option>';
    $html .= '<option value="3">'.esc_html__('Kritisch', 'cp-community').'</option>';
    $html .= '</select>';
    $html .= '<select class="cpc_projects_task_filter_overdue">';
    $html .= '<option value="0">'.esc_html__('Fristen', 'cp-community').'</option>';
    $html .= '<option value="1">'.esc_html__('Überfällig', 'cp-community').'</option>';
    $html .= '</select>';
    $html .= '<select class="cpc_projects_task_filter_assignee">';
    $html .= '<option value="all">'.esc_html__('Zugewiesen', 'cp-community').'</option>';
    ...
    if ($can_manage) {
        ...
        $html .= '<summary class="cpc_button cpc_projects_task_form_summary">'.esc_html__('+ Task hinzufuegen', 'cp-community').'</summary>';
        ...
        $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Titel', 'cp-community').'</label>';
        $html .= '<input type="text" name="title" placeholder="'.esc_attr__('Task-Titel', 'cp-community').'" required />';
        $html .= '</div>';
        $html .= '<div class="cpc_projects_task_field">';
        $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Prioritaet', 'cp-community').'</label>';
        ...
        $html .= '</select>';
        $html .= '</div>';
        $html .= '<button type="submit" class="cpc_button cpc_projects_add_task_btn">'.esc_html__('Task hinzufuegen', 'cp-community').'</button>';
        $html .= '</div>';
        $html .= '<div class="cpc_projects_task_form_row cpc_projects_task_form_row_secondary">';
        $html .= '<div class="cpc_projects_task_field">';
        $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Deadline', 'cp-community').'</label>';
        $html .= '<input type="datetime-local" name="deadline" placeholder="'.esc_attr__('Deadline', 'cp-community').'" value="'.esc_attr(cpc_projects_get_default_deadline_datetime()).'" />';
        $html .= '</div>';
        $html .= '<div class="cpc_projects_task_field">';
        $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Zuweisen an', 'cp-community').'</label>';
        ...
        $html .= '</div>';
        $html .= '</div>';
        $html .= '<div class="cpc_projects_task_hint">'.esc_html__('Zuweisung (Mehrfachauswahl moeglich)', 'cp-community').'</div>';
        $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Beschreibung', 'cp-community').'</label>';
        $html .= '<textarea name="description" rows="2" placeholder="'.esc_attr__('Kurzbeschreibung (optional)', 'cp-community').'"></textarea>';
        if (cpc_projects_task_comment_attachments_enabled()) {
            $html .= '<div class="cpc_projects_task_file_upload">';
            $html .= '<label class="cpc_projects_task_file_label">'.esc_html__('Datei anhaengen (optional)', 'cp-community').'</label>';
    ...
    if (empty($tasks)) {
        $html .= '<p class="cpc_projects_no_tasks">'.esc_html__('Noch keine Tasks vorhanden.', 'cp-community').'</p>';
    } else {
        ...
        foreach ($tasks as $task) {
            ...
            $html .= '<span class="cpc_projects_task_status status-'.esc_attr($is_done ? 'done' : 'open').'">'.esc_html($is_done ? __('Erledigt', 'cp-community') : __('Offen', 'cp-community')).'</span>';
            ...
            if (!empty($task->deadline)) {
                ...
                if ($deadline_ts_meta > $now_ts_meta && !$is_done) {
                    $countdown_str = sprintf(__('(%s verbleibend)', 'cp-community'), human_time_diff($now_ts_meta, $deadline_ts_meta));
                } elseif ($deadline_ts_meta <= $now_ts_meta && !$is_done) {
                    $countdown_str = __('(UEBERFAELLIG)', 'cp-community');
                } else {
                    $countdown_str = '';
                }
                $task_meta_bits[] = sprintf(__('Faellig: %s', 'cp-community'), $deadline_date_str.(($countdown_str !== '') ? ' '.$countdown_str : ''));
            }
            ...
            if (!empty($task_assigned_ids)) {
                ...
                if (!empty($assigned_names)) {
                    $task_meta_bits[] = sprintf(__('Zugewiesen: %s', 'cp-community'), implode(', ', $assigned_names));
                }
            }
            ...
            if ($can_manage) {
                ...
                $html .= '<button type="button" class="cpc_projects_task_delete cpc_projects_inline_link" data-task-id="'.(int)$task->id.'">'.esc_html__('Loeschen', 'cp-community').'</button>';
                $html .= '<button type="button" class="cpc_projects_task_edit_toggle cpc_projects_inline_link" data-task-id="'.(int)$task->id.'">'.esc_html__('Bearbeiten', 'cp-community').'</button>';
                $html .= '<button type="button" class="cpc_projects_task_details_toggle cpc_projects_inline_link" data-task-id="'.(int)$task->id.'">'.esc_html__('Details', 'cp-community').'</button>';
                $html .= '</div>';
                ...
                $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Prioritaet', 'cp-community').'</label>';
                $html .= '<select name="priority">';
                $html .= '<option value="1" '.selected((int)$task->priority, 1, false).'>'.esc_html__('Normal', 'cp-community').'</option>';
                $html .= '<option value="2" '.selected((int)$task->priority, 2, false).'>'.esc_html__('Hoch', 'cp-community').'</option>';
                $html .= '<option value="3" '.selected((int)$task->priority, 3, false).'>'.esc_html__('Kritisch', 'cp-community').'</option>';
                $html .= '</select>';
                $html .= '<label class="cpc_projects_task_field_label">'.esc_html__('Deadline', 'cp-community').'</label>';
                ...
                if (cpc_projects_task_comment_attachments_enabled()) {
                    $html .= '<div class="cpc_projects_task_file_upload">';
                    $html .= '<label class="cpc_projects_task_file_label">'.esc_html__('Weitere Datei anhaengen (optional)', 'cp-community').'</label>';
                    $html .= '<input type="file" name="task_attachments[]" class="cpc_projects_task_files" multiple />';
                    $html .= '</div>';
                }
                $html .= '<button type="submit" class="cpc_projects_inline_link">'.esc_html__('Speichern', 'cp-community').'</button>';
                $html .= '</form>';
            }
            ...
            $html .= '<div><strong>'.esc_html__('Erstellt', 'cp-community').':</strong> '.esc_html(wp_date(get_option('date_format').' '.get_option('time_format'), strtotime((string)$task->date_added))).'</div>';
            $html .= '<div><strong>'.esc_html__('Zuletzt geaendert', 'cp-community').':</strong> '.esc_html(wp_date(get_option('date_format').' '.get_option('time_format'), strtotime((string)$task->date_updated))).'</div>';
            if ($task_creator) {
                $html .= '<div><strong>'.esc_html__('Ersteller', 'cp-community').':</strong> '.esc_html($task_creator->display_name).'</div>';
            }
            if ($task_completer) {
                $html .= '<div><strong>'.esc_html__('Abgeschlossen von', 'cp-community').':</strong> '.esc_html($task_completer->display_name).'</div>';
            }
            if (!empty($task_assigned_ids)) {
                $html .= '<div class="cpc_projects_task_assignee_avatars"><strong>'.esc_html__('Zugewiesen an', 'cp-community').':</strong>';
            ...
            $html .= '<div><strong>'.esc_html__('Kommentare', 'cp-community').':</strong> '.(int)count($comments).'</div>';
            $html .= '</div>';
            ...
            if (!empty($task_direct_attachments)) {
                ...
                $html .= '<h6>'.esc_html__('Dateianhaenge', 'cp-community').'</h6>';
                foreach ($task_direct_attachments as $att_id) {
                    ...
                    if ($can_manage) {
                        $html .= ' <button type="button" class="cpc_projects_task_direct_attachment_delete cpc_projects_inline_link" data-attachment-id="'.(int)$att_id.'">'.esc_html__('Loeschen', 'cp-community').'</button>';
                    }
                    $html .= '</div>';
                }
                $html .= '</div>';
            ...
            if (!empty($task_events)) {
                ...
                $html .= '<h6>'.esc_html__('Verlauf', 'cp-community').'</h6>';
                $html .= '<ul>';
                foreach ($task_events as $event) {
                    $event_author = $event->comment_author ? $event->comment_author : __('Mitglied', 'cp-community');
                    $event_time = sprintf(__('vor %s', 'cp-community'), human_time_diff(strtotime($event->comment_date_gmt), current_time('timestamp', 1)));
            ...
            $html .= '<h6 class="cpc_projects_discussion_heading">'.esc_html__('Diskussion', 'cp-community').'</h6>';
            if (!empty($comments)) {
                $html .= '<ul class="cpc_projects_task_comment_list">';
                foreach ($comments as $comment) {
                    $author_name = $comment->comment_author ? $comment->comment_author : __('Mitglied', 'cp-community');
                    $time = sprintf(__('vor %s', 'cp-community'), human_time_diff(strtotime($comment->comment_date_gmt), current_time('timestamp', 1)));
                    ...
                    if ($can_delete_comment) {
                        $html .= '<button type="button" class="cpc_projects_comment_delete cpc_projects_inline_link" data-comment-id="'.(int)$comment->comment_ID.'">'.esc_html__('Loeschen', 'cp-community').'</button>';
                    }
                    $html .= '</div>';
                    ...
                    if (!empty($attachment_ids)) {
                        ...
                        foreach ($attachment_ids as $attachment_id) {
                            ...
                            if (cpc_projects_user_can_delete_comment_attachment($attachment_id, get_current_user_id())) {
                                $html .= '<button type="button" class="cpc_projects_comment_attachment_delete cpc_projects_inline_link" data-attachment-id="'.(int)$attachment_id.'">'.esc_html__('Datei loeschen', 'cp-community').'</button>';
                            }
                        }
                        $html .= '</div>';
                    }
            ...
            if (is_user_logged_in() && cpc_projects_user_can_view_project($project_id)) {
                ...
                $html .= '<textarea name="comment" rows="2" maxlength="1500" placeholder="'.esc_attr__('Kommentar schreiben...', 'cp-community').'" required></textarea>';
                if (cpc_projects_task_comment_attachments_enabled()) {
                    $html .= '<input type="file" name="attachments[]" class="cpc_projects_task_comment_files" multiple />';
                }
                $html .= '<button type="submit" class="cpc_projects_inline_link">'.esc_html__('Kommentieren', 'cp-community').'</button>';
                $html .= '</form>';
            }
            $html .= '</div>';
        ...
        $html .= '</div>';
        $html .= '<p class="cpc_projects_no_tasks cpc_projects_no_tasks_filtered" style="display:none">'.esc_html__('Keine passenden Tasks gefunden.', 'cp-community').'</p>';
    }
    $html .= '</div>';
    $html .= '</div>';
...
function cpc_projects_render_project_progress($project_id) {
    ...
    $html .= sprintf(
        esc_html__('%1$s Tasks, %2$s%% erledigt (%3$s offen)', 'cp-community'),
        (int)$stats['total'],
        (int)$stats['progress'],
        (int)$stats['remaining']
    );
...
function cpc_projects_render_projects_list($projects, $args = array()) {
    ...
    if (empty($projects)) {
        return '<p id="message" class="info">'.esc_html__('Derzeit wurden keine Projekte gefunden.', 'cp-community').'</p>';
    }
...
function cpc_projects_render_profile_tab_content($html, $active_tab, $user_id, $shortcode_atts) {
    ...
    if (get_current_user_id() === $user_id) {
        ...
        $html .= '<h5>'.esc_html__('E-Mail-Benachrichtigungen', 'cp-community').'</h5>';
        $html .= '<form class="cpc_projects_notification_prefs_form">';
        $html .= '<div class="cpc_projects_prefs_notice" style="display:none"></div>';
        $html .= '<label class="cpc_projects_pref_label"><input type="checkbox" name="notify_task" value="1" '.checked($notify_task, 1, false).' /> '.esc_html__('Bei neuer Aufgabe benachrichtigen', 'cp-community').'</label>';
        $html .= '<label class="cpc_projects_pref_label"><input type="checkbox" name="notify_comment" value="1" '.checked($notify_comment, 1, false).' /> '.esc_html__('Bei neuem Kommentar benachrichtigen', 'cp-community').'</label>';
        $html .= '<p><button type="submit" class="cpc_button">'.esc_html__('Einstellungen speichern', 'cp-community').'</button></p>';
        $html .= '</form>';
        $html .= '</div>';
    }
...
function cpc_projects_render_group_tab_content($html, $group_id, $shortcode_atts) {
    ...
    $html .= cpc_projects_render_notice_html();
    $html .= '<div class="cpc_projects_group_tab">';
    $html .= '<h3>'.esc_html__('Gruppen-Projekte', 'cp-community').'</h3>';
...
function cpc_projects_directory_shortcode($atts) {
    ...
    if (empty($results['items'])) {
        $html .= '<p>'.esc_html__('Keine Projekte gefunden.', 'cp-community').'</p>';
    } else {
...
function cpc_projects_render_directory_filters($search = '') {
    ...
    $html .= '<input type="search" name="cpc_projects_q" value="'.esc_attr($search).'" placeholder="'.esc_attr__('Titel suchen...', 'cp-community').'" />';
    $html .= '<button type="submit" class="cpc_button">'.esc_html__('Suchen', 'cp-community').'</button>';
    $html .= '</form>';
    return $html;
}
...
function cpc_projects_render_directory_pagination($results, $page, $per_page, $search = '') {
    ...
    if ($page > 1) {
        $html .= '<a class="cpc_button" href="'.esc_url(add_query_arg('cpc_projects_page', $page - 1, $base_url)).'">← '.esc_html__('Vorherige', 'cp-community').'</a>';
    }
    $html .= '<span class="cpc_projects_directory_page_label">'.sprintf(esc_html__('Seite %d', 'cp-community'), $page).'</span>';
    if ($results['has_more']) {
        $html .= '<a class="cpc_button" href="'.esc_url(add_query_arg('cpc_projects_page', $page + 1, $base_url)).'">'.esc_html__('Nächste', 'cp-community').' →</a>';
    }
    $html .= '</div>';
    return $html;
}
