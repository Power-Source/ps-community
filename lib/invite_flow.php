...
function cpc_invite_send_email($email, $token, $redirect_url, $inviter_id) {
    ...
    $inviter_name = $inviter ? $inviter->display_name : __('Ein Mitglied', 'cp-community');

    $subject = sprintf(__('%s hat Dich in die Community eingeladen', 'cp-community'), $inviter_name);
    $message = '';
    $message .= sprintf(__('Hallo! %s hat Dich eingeladen.', 'cp-community'), $inviter_name) . "\n\n";
    $message .= __('Öffne diesen Link, um Einladung und Weiterleitung sicher zu übernehmen:', 'cp-community') . "\n";
    $message .= $invite_link . "\n\n";
    $message .= __('Nach dem Login wirst Du automatisch an das Ziel weitergeleitet.', 'cp-community') . "\n";
    $message .= __('Geplantes Ziel:', 'cp-community') . ' ' . cpc_invite_safe_redirect($redirect_url) . "\n";
...
function cpc_invite_handle_token() {
    ...
    if (!is_array($payload)) {
        wp_die(esc_html__('Der Einladungslink ist ungültig oder abgelaufen.', 'cp-community'));
    }
    ...
    if (is_user_logged_in()) {
        ...
        if ($expected_email !== '' && $expected_email !== $current_email && !current_user_can('manage_options')) {
            wp_die(esc_html__('Diese Einladung ist für eine andere E-Mail-Adresse vorgesehen.', 'cp-community'));
        }
    ...
    $html .= '<h2>' . esc_html__('Einladung erkannt', 'cp-community') . '</h2>';
    $html .= '<p>' . esc_html__('Bitte melde Dich an oder registriere Dich. Danach wirst Du automatisch weitergeleitet.', 'cp-community') . '</p>';
    $html .= '<p><a class="button button-primary" href="' . esc_url($login_url) . '">' . esc_html__('Anmelden', 'cp-community') . '</a> ';
    $html .= '<a class="button" href="' . esc_url($register_url) . '">' . esc_html__('Registrieren', 'cp-community') . '</a></p>';
    $html .= '</div>';
...
function cpc_invite_shortcode($atts) {
    ...
    if ($status === 'sent') {
        $notice = '<div class="cpc_success">' . esc_html__('Einladung wurde verschickt.', 'cp-community') . '</div>';
    } elseif ($status === 'send_failed') {
        $notice = '<div class="cpc_error">' . esc_html__('Einladung konnte nicht versendet werden.', 'cp-community') . '</div>';
    } elseif ($status === 'invalid_email') {
        $notice = '<div class="cpc_error">' . esc_html__('Bitte gib eine gültige E-Mail-Adresse ein.', 'cp-community') . '</div>';
    }
    ...
    $html .= '<p><label>' . esc_html__('E-Mail-Adresse', 'cp-community') . '<br>';
    $html .= '<input type="email" name="cpc_invite_email" required></label></p>';
    $html .= '<p><label>' . esc_html__('Weiterleitungsziel (gleiches Netzwerk)', 'cp-community') . '<br>';
